...
| status      | TINYINT UNSIGNED | 文章状态(0草稿,1已发布) |
...
**注意：** 为遵循数据库第三范式，点赞数据已移至独立的点赞表（post_likes）中管理。
...
### 2.7 文章点赞表（`post_likes`）

| 字段名     | 类型             | 备注                           |
| ---------- | ---------------- | ------------------------------ |
| id         | BIGINT PK        | 自增主键                       |
| user_id    | BIGINT FK        | 外键，用户ID                   |
| post_id    | BIGINT FK        | 外键，文章ID                   |
| is_like    | TINYINT UNSIGNED | 是否点赞(0取消点赞,1点赞)      |
| created_at | TIMESTAMP        | 创建时间                       |
| updated_at | TIMESTAMP        | 更新时间                       |

**索引设计：**
- 唯一索引：`uk_user_post (user_id, post_id)` - 确保用户对同一文章只能有一条点赞记录
- 普通索引：`idx_user_id (user_id)` - 优化按用户查询
- 普通索引：`idx_post_id (post_id)` - 优化按文章查询

------

### 2.8 文章收藏表（`post_collections`）
...
| user_id    | BIGINT FK        | 外键，用户ID                   |
| post_id    | BIGINT FK        | 外键，文章ID                   |
| is_collect | TINYINT UNSIGNED | 是否收藏(0取消收藏,1收藏)      |
| created_at | TIMESTAMP        | 创建时间                       |
| updated_at | TIMESTAMP        | 更新时间                       |

**索引设计：**
- 唯一索引：`uk_user_post (user_id, post_id)` - 确保用户对同一文章只能有一条收藏记录
- 普通索引：`idx_user_id (user_id)` - 优化按用户查询
- 普通索引：`idx_post_id (post_id)` - 优化按文章查询
...
| content     | TEXT             | 公告内容                       |
| type        | TINYINT UNSIGNED | 公告类型(1系统,2活动,3维护,4其他) |
| priority    | TINYINT UNSIGNED | 优先级(1低,2中,3高,4紧急)        |
| status      | TINYINT UNSIGNED | 状态(0草稿,1发布,2下线)          |
| start_time  | TIMESTAMP NULL   | 开始显示时间                   |
| end_time    | TIMESTAMP NULL   | 结束显示时间                   |
| is_top      | TINYINT UNSIGNED | 是否置顶(0否,1是)              |
| view_count  | INT UNSIGNED     | 查看次数                       |
| created_at  | TIMESTAMP        | 创建时间                       |
| updated_at  | TIMESTAMP        | 更新时间                       |
| created_by  | BIGINT NULL      | 创建人ID                       |
| updated_by  | BIGINT NULL      | 更新人ID                       |
...
Table post_collections {
  ...
  user_id bigint [ref: > users.id]
  post_id bigint [ref: > posts.id]
  is_collect tinyint [not null, default: 1]
  created_at timestamp [not null]
  updated_at timestamp [not null]
  indexes {
    (user_id, post_id) [unique]
...
CREATE TABLE post_likes (
  ...
  post_id BIGINT NOT NULL COMMENT '文章ID',
  is_liked TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '点赞状态(0取消点赞,1点赞)',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE post_collections (
  ...
  user_id BIGINT NOT NULL COMMENT '用户ID',
  post_id BIGINT NOT NULL COMMENT '文章ID',
  is_collected TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '收藏状态(0取消收藏,1收藏)',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  UNIQUE KEY uk_user_post (user_id, post_id),
  INDEX idx_post_id (post_id),
