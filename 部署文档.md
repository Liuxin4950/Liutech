...
![部署状态](https://img.shields.io/badge/部署-Docker%20支持-2496ED?style=flat-square&logo=docker)
![环境](https://img.shields.io/badge/环境-生产就绪-green?style=flat-square)
...
LiuTech 是一个现代化的全栈博客系统，采用前后端分离架构，支持文章发布、分类管理、标签系统、用户认证、文件上传、AI聊天助手等功能。系统包含用户前台展示和管理后台两个前端应用，以及统一的后端API服务。
...
- **构建工具**：Vite 7.1.3
- **状态管理**：Pinia 3.0.3 + 持久化插件 4.4.1
- **路由管理**：Vue Router 4.5.1
- **HTTP 客户端**：Axios 1.10.0
- **富文本编辑**：TinyMCE 7.9.1 + Vue 组件 6.2.0
- **样式预处理**：Sass Embedded 1.90.0
- **交互组件**：SweetAlert2 11.22.2
- **动画效果**：Pixi.js 6.5.10 + Live2D 0.4.0
...
- **JDK 版本**：Java 21
- **数据库连接**：MySQL Connector/J 8.3.0
- **安全框架**：Spring Security + JWT
- **缓存支持**：Spring Data Redis
- **监控工具**：Spring Boot Actuator
- **构建工具**：Maven 3.x
- **容器化**：Docker + Docker Compose

### 2.3 部署架构
- **用户前端**：http://localhost:3000 (开发) / http://web.localhost (Docker)
- **管理后台**：http://admin.localhost (Docker)
- **后端API**：http://localhost:8080 (Spring Boot内嵌Tomcat)
- **数据库**：MySQL 8.0+ (Docker容器 / 云服务器)
- **反向代理**：Nginx (Docker容器)
- **文件存储**：本地文件系统 + 静态资源服务
